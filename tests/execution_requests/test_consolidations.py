...
def test_consolidation_foreign_source_and_target_pubkey_from_user_withdrawal_address(withdrawal_address: str, watcher: WatcherStub):
    ...
    general_user_withdrawal_address_alert = next((alert from alert in watcher.alertmanager.sent_alerts if alert.labels.alertname.startswith('HeadWatcherConsolidationSourceWithdrawalAddress')), None)
    ...
    assert (
        general_user_withdrawal_address_alert.annotations.summary == "🚨🚨🚨 Validator consolidation was requested from Withdrawal Vault source address"
    )
    ...
    foreign_source_alert = next((alert for alert in watcher.alertmanager.sent_alerts if alert.labels.alertname.startswith('HeadWatcherConsolidationUserWithdrawalAddressForeignSourcePubkey')), None)
    assert foreign_source_alert is not None

    foreign_target_alert = next((alert for alert in watcher.alertmanager.sent_alerts if alert.labels.alertname.startswith('HeadWatcherConsolidationUserWithdrawalAddressForeignTargetPubkey')), None)
    assert foreign_target_alert is not None


def test_consolidation_foreign_source_pubkey_from_user_withdrawal_address(withdrawal_address: str, user_validator_2: TestValidator, watcher: WatcherStub):
    random_source_pubkey = gen_random_pubkey()
    ...
    user_wa_alert = next((alert for alert in watcher.alertmanager.sent_alerts if alert.labels.alertname.startswith('HeadWatcherConsolidationSourceWithdrawalAddress')), None)
    ...
    assert (
        foreign_source_alert.annotations.summary == "🚨🚨🚨 Validator consolidation was requested for foreign source validator from Withdrawal Vault address"
    )
...
def test_consolidation_foreign_target_pubkey_from_user_withdrawal_address(withdrawal_address: str, user_validator_1: TestValidator, watcher: WatcherStub):
    random_target_pubkey = gen_random_pubkey()
    ...
    assert (
        foreign_target_alert.annotations.summary == "🚨🚨🚨 Validator consolidation was requested from Withdrawal Vault address to foreign target validator"
    )
...
def test_consolidation_foreign_withdrawal_address_user_source_pubkey(user_validator_1: TestValidator, watcher: WatcherStub):
    ...
    assert alert.annotations.summary == "⚠️⚠️⚠️ Consolidation was requested for our validators (not from Withdrawal Vault address)"
...
def test_consolidation_foreign_withdrawal_address_user_target_pubkey(user_validator_2: TestValidator, watcher: WatcherStub):
    random_source_address = gen_random_address()
    random_source_pubkey = gen_random_pubkey()
    ...
    assert alert.labels.severity == 'info'
    assert alert.annotations.summary == "⚠️⚠️⚠️ Someone attempts to consolidate their validators to our validators (not from Withdrawal Vault address)"
    assert random_source_address in alert.annotations.description
