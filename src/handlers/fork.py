...
class ForkHandler(WatcherHandler):
    ...
    def handle(self, watcher, head: FullBlockInfo):
        def _known_block(root: str) -> FullBlockInfo:
            block, *_ = [h for h in [*watcher.handled_blocks, head] if h.root == root] or [None]
            return block
        ...
        for chain_reorg in chain_reorgs:
            known_block = _known_block(chain_reorg.new_head_block)
            known_parent = _known_block(known_block.header.message.parent_root) if known_block else None
            if not known_block or not known_parent:
        ...
        if watcher.handled_blocks and not head_parent_is_alerted:
            known_parent = _known_block(head.header.message.parent_root)
            if not known_parent:
    ...
    def _send_unhandled_head_alert(self, watcher, head: FullBlockInfo):
        alert = CommonAlert(name="UnhandledHead", severity="info")
        summary = "**🫳🐦 Unhandled chain slot**"
        additional_msg = ""
        diff = int(head.header.message.slot) - int(watcher.handled_blocks[-1].header.message.slot) - 2
        if diff > 0:
